...
private _allowUnitFnc =
{
    params ["_entity"];

    _entity setVariable ["WR_interactionAllowed", true, true];
};
...
if (_unitsScope isEqualTo "GLOBAL") then
{
    ...
    private _eventFunc = { params ["_entity"]; [_entity] call _allowUnitFnc; };
...
if (_unitsScope isEqualTo "SYNCED") then
{
    {
        if (_x isKindOf "Man") then { [_x] call _allowUnitFnc; };
    } forEach _syncedEntities;
};

/* ================================================================================ */

private _allowEntityFnc =
{
    params ["_entity"];
...
if (_assetsScope isEqualTo "SYNCED") then
{
    {
        private _class = typeOf _x;
        if (_class in _supportedAssetClasses) then { [_x] call _allowEntityFnc; };
    } forEach _syncedEntities;
};
